...
vec3 normal(vec3 p) {
    vec2 q = vec2(0., EPS);
    return normalize(vec3(mapH(p + q.yxx) - mapH(p - q.yxx), mapH(p + q.xyx) - mapH(p - q.xyx), mapH(p + q.xxy) - mapH(p - q.xxy)));
}

vec3 normalL(vec3 p) {
    vec2 q = vec2(0., EPS);
    return normalize(vec3(mapL(p + q.yxx) - mapL(p - q.yxx), mapL(p + q.xyx) - mapL(p - q.xyx), mapL(p + q.xxy) - mapL(p - q.xxy)));
}
...
vec3 fullSky(vec3 sundir, vec3 dir) {
    ...
    float s = 0.25;
    for(int i = 0; i < 3; ++i) {
        clouds += fbmSimple(dir.xz / (dir.y + EPS) - s);
        s *= 1.35;
    }
...
void main(void) {
    vec2 uv = (-uResolution.xy + 2.0 * gl_FragCoord.xy) / uResolution.y;

    vec3 ro = vec3(0.0, 0.15 + 1.2 * fbmL(vec2(0.0, -0.2)), -0.2);

    vec3 rd = normalize(vec3(uv, -1.0));

    float t = raymarch(ro, rd);

    vec3 col = smoothstep(0.0, 1.0, pow(shade(ro, rd, t), vec3(1.0 / 2.2))) * 1.2;
